...
class BasketConfig extends Component {
  ...
  callSaveAPI() {
    fetch("https://eshopapi.ryananderson.uk/api/basket/save", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
      },
      body: JSON.stringify(this.state.save),
    })
      .then((res) => {
  ...
  callAPI() {
    fetch("https://eshopapi.ryananderson.uk/api/basket/get/all", {
      ...
      .then((res) => {
        fetch("https://eshopapi.ryananderson.uk/api/basket/get", {
          method: "POST",
          headers: {
            Accept: "application/json, text/plain, */*",
            "Content-Type": "application/json",
          },
          body: JSON.stringify(this.state.basketID),
        })
          .then((res) => res.json())
          .then((res) =>
            this.setState({ products: res, loading: false, reloading: false })
          );
      });
  }

  callFindAPI() {
    fetch("https://eshopapi.ryananderson.uk/api/basket/find", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
      ...
      .then((res) => {
        if (res.items === undefined) {
          ...
          for (let x = 0; x < this.state.refine.products.length; x++) {
            i.push({
              term: this.state.refine.products[x],
              found: false,
              item: {
                url: "",
                shop: "",
  ...
  saveBasket(event) {
    ...
    for (let i = 0; i < prods.length; i++) {
      let item = {
        term: prods[i].term,
        item: prods[i].item.url,
        shop: prods[i].item.shop,
      };
      x.push(item);
    }
    this.setState(
    ...
    this.setState(
      {
        ...
        basketID: { id: this.props.match.params.id },
        save: savepre,
      },
      () => {
        this.callAPI();
    ...
    return (
      <div style={{ width: "100%" }}>
        ...
        <Modal
          ...
          overwrite={this.props.match.params.id}
          show={show}
          saveBasket={this.saveBasket}
        ...
        <div className={this.state.reloading ? "hidden" : "price-section"}>
          ...
          <input
            type="submit"
            value="Find Best Basket!"
            className="button"
            form="x1"
            style={{
              width: "max-content",
              margin: "auto 10pt auto auto",
              height: "max-content",
            }}
          ></input>
        </div>
